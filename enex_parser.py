...
class Resource:
    ...
    def md5(self) -> str | None:
        """Return the hex MD5 hash of the resource data.

        Used by en-media references.
        """
        raw = self.data
        if raw is None:
            return None
...
def _parse_resource(elem: ET.Element) -> Resource:
    ...
    ra = (
        _parse_resource_attributes(ra_elem)
    ...
    if reco_elem is not None:
        ...
        reco = (
            ET.tostring(reco_elem, encoding="unicode")
            if len(reco_elem)
            else _text(reco_elem)
        )
...
def _cli() -> None:
    ...
    for export in exports:
        ...
        for i, note in enumerate(export.notes[:5], 1):
            res_info = (
                f"  [{len(note.resources)} attachment(s)]" if note.resources else ""
            )
            ...
            for r in note.resources[:2]:
                print(
                    f"       resource: {r.resource_attributes.file_name}"
                    f" ({r.mime}, ~{r.size:,} bytes)"
                )
    ...
    print(f"\n{'═' * 60}")
    print(
        f"  TOTAL: {total_notes} notes, {total_resources} resources"
        f" across {len(exports)} file(s)"
    )
    print(f"{'═' * 60}\n")
